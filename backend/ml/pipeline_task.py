...
async def _merge_or_create_spot(
    ...
    # Use PostGIS ST_Intersects + ST_Area for IoU check
    result = await db.execute(
        text("""
            SELECT id, geom
            FROM spots
...
async def run_pipeline(
    ...
    3. Run inference → change probability mask
    4. Postprocess → binary mask → regions → filter by area
    5. Classify each region
    6. Merge with existing spots (IoU) or create new spot
    7. Persist Detection records
    ...
    for interval in INTERVALS:
        ...
        try:
            ...
            prob_mask = run_inference(before_path, after_path)
